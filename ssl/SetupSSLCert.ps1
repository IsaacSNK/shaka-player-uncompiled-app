...
.DESCRIPTION
    ...
    Additionally checks if the hosts file entry for localhost exists and adds it if necessary.
...
.PARAMETER localhostIp
    ...
    This is used for setting up proper host inside docker so the host dev server can be called properly through localhost.
...
Write-Host "`n======================================"
Write-Host "Setup localhost SSL cert and host mapping"
Write-Host "======================================`n"
...
if ($force) {
    ...
    Write-Host "`n======================================"
    Write-Host "localhost is already mapped in host file and cert installed. Exit early."
    Write-Host "======================================"
    return
}
...
if (!$hasHostMapping) {
    # Add localhost to hosts file but remove any previous entries first
    $newFileContent = ""
    if ($hostFileContent) {
        Write-Host "`n======================================"
